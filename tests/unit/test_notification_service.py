...
def test_should_send_m1_fired_telegram_accepts_strong_ratio_and_volume():
    alert = {
        "status": "fired",
        "ticker": "VNM",
        "ratio_5d": 5.4,
...
@pytest.mark.asyncio
async def test_send_volume_alert_email_suppresses_repeat_without_stronger_move(mock_pool, mock_redis):
    pool, conn = mock_pool
    notification.inject_deps(pool, mock_redis)
    mock_redis.hgetall = AsyncMock(return_value={b"ratio": b"6.0", b"volume": b"80000"})
    conn.fetchrow = AsyncMock(return_value={
        "id": 11,
        "ticker": "ACB",
        "slot": 30,
        "fired_at": datetime(2026, 3, 31, 2, 30, tzinfo=timezone.utc),
        "bar_time": datetime(2026, 3, 31, 2, 30, tzinfo=timezone.utc),
        "status": "fired",
        "ratio_5d": 6.2,
        "volume": 85_000,
        "baseline_5d": 10_000,
    ...
    with patch("app.services.notification._send_email", new=AsyncMock()) as mock_email, \
         patch("app.services.notification._send_telegram", new=AsyncMock()) as mock_tg:
        await notification.send_volume_alert_email(11)
...
@pytest.mark.asyncio
async def test_send_volume_alert_email_sends_repeat_when_signal_stronger(mock_pool, mock_redis):
    pool, conn = mock_pool
    notification.inject_deps(pool, mock_redis)
    mock_redis.hgetall = AsyncMock(return_value={b"ratio": b"6.0", b"volume": b"80000"})
    conn.fetchrow = AsyncMock(return_value={
        ...
        "ticker": "ACB",
        "slot": 31,
        "fired_at": datetime(2026, 3, 31, 2, 31, tzinfo=timezone.utc),
        "bar_time": datetime(2026, 3, 31, 2, 31, tzinfo=timezone.utc),
        "status": "fired",
        "ratio_5d": 9.5,
        "volume": 90_000,
        "baseline_5d": 10_000,
    ...
    with patch("app.services.notification._send_email", new=AsyncMock()) as mock_email, \
         patch("app.services.notification._send_telegram", new=AsyncMock()) as mock_tg:
        await notification.send_volume_alert_email(12)
...
@pytest.mark.asyncio
async def test_send_volume_alert_confirmation_suppresses_repeat_without_progression(mock_pool, mock_redis):
    pool, conn = mock_pool
    notification.inject_deps(pool, mock_redis)
    mock_redis.hgetall = AsyncMock(return_value={b"ratio": b"1.6", b"volume": b"30000"})
    conn.fetchrow = AsyncMock(return_value={
        "id": 22,
        "ticker": "VCB",
        "slot": 35,
    ...
    with patch("app.services.notification._send_telegram", new=AsyncMock()) as mock_tg:
        await notification.send_volume_alert_confirmation(22)
